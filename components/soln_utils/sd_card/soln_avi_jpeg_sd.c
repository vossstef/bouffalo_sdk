...
#ifdef CONFIG_SOLN_SD_AVI_LOG_LEVEL
#undef CONFIG_LOG_LEVEL
#define CONFIG_LOG_LEVEL CONFIG_SOLN_SD_AVI_LOG_LEVEL
#endif
#define DBG_TAG "SOLN_SD_AVI"
#include "log.h"

#if IS_ENABLED(CONFIG_SOLN_SD_AVI_VIDEO_EN)
static uint32_t s_avi_sd_total_frame_count = 0;

static void avi_jpeg_sd_record_frame(void)
{
    uintptr_t irq_flags = bflb_irq_save();
    s_avi_sd_total_frame_count++;
    bflb_irq_restore(irq_flags);
}
#endif

uint32_t soln_avi_jpeg_sd_get_total_frame_count(void)
{
#if IS_ENABLED(CONFIG_SOLN_SD_AVI_VIDEO_EN)
    uintptr_t irq_flags = bflb_irq_save();
    uint32_t frame_count = s_avi_sd_total_frame_count;
    bflb_irq_restore(irq_flags);
...
void soln_avi_jpeg_sd_clear_total_frame_count(void)
{
#if IS_ENABLED(CONFIG_SOLN_SD_AVI_VIDEO_EN)
    uintptr_t irq_flags = bflb_irq_save();
    s_avi_sd_total_frame_count = 0;
    bflb_irq_restore(irq_flags);
#endif
}
...
static FRESULT psram_f_write(FIL *fp, const void *buff, UINT btw, UINT *bw)
{
    FRESULT ret;
    UINT btw_c, bw_c = 0;
...
static int avi_build_header(avi_file_t *avi_hd)
{
    ...
    LOG_I("header data size %d\r\n", avi_hd->head_data_size);
    LOG_I("header junk size %d\r\n", junk_size);
    LOG_I("header sector num %d\r\n", avi_hd->head_data_size / 512);

    /* save the sector_num */
    avi_hd->disk_head_sector_num = avi_hd->head_data_size / 512;

    if (avi_hd->head_data_size > avi_hd->head_buff_size) {
        LOG_E("head size size over: %d -> %d\r\n", avi_hd->head_data_size, avi_hd->head_buff_size);
        return -1;
    }
...
static int avi_build_subindex(avi_file_t *avi_hd)
{
    ...
    if (avi_hd->avi_param.auds_en) {
        ...
        if (avi_hd->sub_idx_data_size > avi_hd->sub_idx_buff_size) {
            LOG_E("sub_idx auds size over %d->%d\r\n", avi_hd->sub_idx_data_size, avi_hd->sub_idx_buff_size);
            return -3;
        }
    ...
    if (avi_hd->sub_idx_data_size > avi_hd->sub_idx_buff_size) {
        LOG_E("sub_idx junk size over %d->%d\r\n", avi_hd->sub_idx_data_size, avi_hd->sub_idx_buff_size);
        return -4;
    }
    ...
    LOG_I("sub idx data size %d\r\n", avi_hd->sub_idx_data_size);
    LOG_I("sub idx junk size %d\r\n", junk_size);
    LOG_I("sub idx sector num %d\r\n", avi_hd->disk_sub_idx_sector_num);
...
/*
*/
static int avi_jpeg_sub_idx_create(avi_file_t *avi_hd)
{
    FRESULT ret;
    UINT bw;
    ...
    */
    ...
    if (junk_size) {
        ...
        if (ret != FR_OK) {
            LOG_E("sub_idx create error, f_write junk, code %d\r\n", ret);
            return -2;
        }
    ...
    if (ret != FR_OK) {
        LOG_E("sub_idx create error, f_write sub_idx, code %d\r\n", ret);
        return -2;
    }
    /* save offset */
...
*/
avi_file_t *soln_avi_file_create(FIL *p_file, avi_param_t *avi_param)
{
    FRESULT ret;
    UINT bw;
    ...
    if (g_fatfs_fs.csize * 512 < AVI_HEAD_BUFF) {
        LOG_E("avi create error, fs->csize");
        return NULL;
    }
    ...
    if (ret != FR_OK) {
        LOG_E("avi create error, f_write, code %d\r\n", ret);
        return NULL;
    }
    /* update ptr */
 ...
 */
int soln_avi_jpeg_file_sync(avi_file_t *avi_hd, uint8_t mode)
{
    FRESULT ret;
    ...
    if (mode & 0x01) {
        ...
        if (avi_hd->disk_head_sector_num * 512 != avi_hd->head_data_size) {
            LOG_E("header_sync, size error %d\r\n", avi_hd->head_data_size);
            return -3;
        }
        ...
        */
        ...
        if (ret != FR_OK) {
            LOG_E("header_sync, disk_write error %d\r\n", ret);
            return -4;
        }
    }
    ...
    if (mode & 0x02) {
        ...
        if (avi_hd->disk_sub_idx_sector_num * 512 != avi_hd->sub_idx_data_size) {
            LOG_E("sub_idx_sync, size error %d\r\n", avi_hd->sub_idx_data_size);
            return -6;
        }
        ...
        */
        ...
        if (ret != FR_OK) {
            LOG_E("sub_idx_sync, disk_write error %d\r\n", ret);
            return -7;
        }
    }
 ...
 */
int soln_avi_write_frame(avi_file_t *avi_hd, void *jpeg_frame, uint32_t frame_size, void *pcm_frame)
{
    FRESULT ret;
    UINT bw;
    ...
    /* check file fptr */
    if (avi_hd->avi_file->fptr != avi_hd->new_chunk_offset) {
        LOG_E("write_frame fprt error: %lld->%lld\r\n", avi_hd->avi_file->fptr, avi_hd->new_chunk_offset);
        return -1;
    }
    ...
    if (avi_hd->avi_param.vids_en) {
        ...
        if (jpeg_frame) {
            uint32_t jpeg_size = frame_size;
            frame_size = (frame_size + 31) & 0xffffffe0;

            LOG_D("write vids new frame, size %d\r\n", frame_size);
            ...
            if (ret != FR_OK || bw != sizeof(CHUNK)) {
                LOG_E("avi_write vids head error, f_write, code %d\r\n", ret);
                return -4;
            }

            /* write vids chunk data */
            ret = psram_f_write(avi_hd->avi_file, jpeg_frame, jpeg_size, &bw);
            if (ret != FR_OK || bw != jpeg_size) {
                LOG_E("avi_write vids data error, f_write, code %d\r\n", ret);
                return -6;
            }
            if (frame_size > jpeg_size) {
                static const uint8_t zero_padding[32] = { 0 };
                uint32_t padding_size = frame_size - jpeg_size;

                ret = f_write(avi_hd->avi_file, zero_padding, padding_size, &bw);
                if (ret != FR_OK || bw != padding_size) {
                    LOG_E("avi_write vids padding error, f_write, code %d\r\n", ret);
                    return -6;
                }
            }
    ...
    if (avi_hd->avi_param.auds_en) {
        ...
        if (pcm_frame) {
            LOG_D("write auds new frame, size %d\r\n", avi_hd->auds_frame_size);
            ...
            ret = f_write(avi_hd->avi_file, &movi_data, sizeof(CHUNK), &bw);
            if (ret != FR_OK || bw != sizeof(CHUNK)) {
                LOG_E("avi_write auds head error, f_write, code %d\r\n", ret);
                return -4;
            }
            ...
            if (ret != FR_OK || bw != avi_hd->auds_frame_size) {
                LOG_E("avi_write auds data error, f_write, code %d\r\n", ret);
                return -6;
            }
...
/* free the mm */
int soln_avi_jpeg_close(avi_file_t *avi_hd)
{
    if (avi_hd == NULL) {
        return -1;
    }

    soln_avi_jpeg_file_sync(avi_hd, (0x01 | 0x02));
...
#if IS_ENABLED(CONFIG_SOLN_SD_AVI_VIDEO_EN)
static fbq_ctrl_t *mjpeg_queue_ctrl_avi = NULL;
static uint16_t mjpeg_queue_depth_avi;
#endif

/* auds and vids(selectable) */
#if IS_ENABLED(CONFIG_SOLN_SD_AVI_AUDIO_EN)

static uint16_t jpeg_output_avi_id;
static uint16_t remote_pcm_output_avi_id;
...
static void save_sdcard_avi_jpeg_pcm_task(void *pvParameters)
{
    ...
    avi_param_t avi_param = {
        ...
        .hight = CONFIG_SOLN_VID_DEFAULT_HEIGHT,
#else
        .vids_en = 0,
#endif
        .auds_en = 1,
...
create_new_file:
    ...
    if (f_ret != FR_OK) {
        LOG_E("avi open failed, num %d, code %d\r\n", file_num, f_ret);
        vTaskSuspend(NULL);
    }

    avi_hd = soln_avi_file_create(&fnew, &avi_param);
    if (avi_hd == NULL) {
        LOG_E("soln_avi_file_create failed\r\n");
        vTaskSuspend(NULL);
    }
    ...
    while (1) {
        /****** copy to ring buff ******/
        /* pop auds frame */
        ret = fbq_pop(soln_fbq_aud_pcm_remote(), &remote_pcm_frame, remote_pcm_output_avi_id, 1000);
        if (ret != FBQ_OK) {
            /* audac pool empty */
            LOG_I("remote PCM pop timeout %d, continue wait... \r\n", ret);
            continue;
        }

        /* copy to ringbuff */
        if (remote_pcm_frame->size + avi_auds_rb_wptr < AUDS_BUFF_BLOCK_SIZE * AUDS_BUFF_BLOCK_NUM) {
            /*  */
            memcpy(avi_auds_ringbuff + avi_auds_rb_wptr, remote_pcm_frame->data, remote_pcm_frame->size);
            avi_auds_rb_wptr += remote_pcm_frame->size;

        } else {
            uint32_t size_1 = AUDS_BUFF_BLOCK_SIZE * AUDS_BUFF_BLOCK_NUM - avi_auds_rb_wptr;
            memcpy(avi_auds_ringbuff + avi_auds_rb_wptr, remote_pcm_frame->data, size_1);

            uint32_t size_2 = remote_pcm_frame->size - size_1;
            memcpy(avi_auds_ringbuff, (uint8_t *)remote_pcm_frame->data + size_1, size_2);
...
#if IS_ENABLED(CONFIG_SOLN_SD_AVI_VIDEO_EN)
        fbq_elem_t *jpeg_frame;
        if (save_cnt > 0) {
            /* pop vids frame */
            ret = fbq_pop(mjpeg_queue_ctrl_avi, &jpeg_frame, jpeg_output_avi_id, 0);
        } else {
            ret = fbq_pop(mjpeg_queue_ctrl_avi, &jpeg_frame, jpeg_output_avi_id, portMAX_DELAY);
        }
#endif
        save_cnt++;

        if (ret < 0 || !IS_ENABLED(CONFIG_SOLN_SD_AVI_VIDEO_EN)) {
            /* write null frame */
            ret = soln_avi_write_frame(avi_hd, NULL, 0, ((uint8_t *)avi_auds_ringbuff + AUDS_BUFF_BLOCK_SIZE * avi_auds_rb_block_r));
            if (ret < 0) {
                error_cnt++;
                LOG_E("avi write error, sd cnt:%d, err_cnt:%d, err_code:%d, offset %d\r\n", save_cnt, error_cnt, ret, avi_hd->new_chunk_offset);
            }
        }
#if IS_ENABLED(CONFIG_SOLN_SD_AVI_VIDEO_EN)
        else {
            ...
            /* write to avi file */
            ret = soln_avi_write_frame(avi_hd, jpeg_frame->data, jpeg_frame->size,
                                       ((uint8_t *)avi_auds_ringbuff + AUDS_BUFF_BLOCK_SIZE * avi_auds_rb_block_r));
            if (ret < 0) {
                error_cnt++;
                LOG_E("avi write error, sd cnt:%d, err_cnt:%d, err_code:%d, offset %d\r\n", save_cnt, error_cnt, ret, avi_hd->new_chunk_offset);
            }
        ...
        /* sync to disk */
        if (save_cnt % 100 == 0) {
            ret = soln_avi_jpeg_file_sync(avi_hd, (0x01 | 0x02));
            if (ret < 0) {
                LOG_E("soln_avi_jpeg_file_sync error\r\n");
            }
        }

        /* log */
        if (save_cnt % 500 == 0) {
            LOG_I("sd cnt:%d, err_cnt:%d, file:%d\r\n", save_cnt, error_cnt, file_num);
        }
        ...
        if (save_cnt >= (60 * 60 * 50)) {
            save_cnt = 0;
            /* close file */
            soln_avi_jpeg_close(avi_hd);
            f_ret = f_close(&fnew);
            if (f_ret != FR_OK) {
                LOG_E("avi close failed, num %d, code %d\r\n", file_num, f_ret);
            }
...
#elif (CONFIG_SOLN_SD_AVI_VIDEO_EN)
...
create_new_file:
    ...
    while (1) {
        /* pop vids frame */
        if (null_cnt == 0) {
            ret = fbq_pop(mjpeg_queue_ctrl_avi, &jpeg_frame, jpeg_output_avi_id, 1000);
            if (ret != FBQ_OK) {
                LOG_I("[sd_avi] jpeg pop timeout: %d, saved:%d, continue wait... \r\n", ret, save_cnt);
                continue;
            }
        ...
        if (ret < 0) {
            /* write null frame */
            ret = soln_avi_write_frame(avi_hd, NULL, 0, NULL);
            if (ret < 0) {
                error_cnt++;
                LOG_E("avi write error, sd cnt:%d, err_cnt:%d, err_code:%d, offset %d\r\n", save_cnt, error_cnt, ret, avi_hd->new_chunk_offset);
            }
            ...
            /* align size to 32Byte */
            /* write to avi file */
            ret = soln_avi_write_frame(avi_hd, jpeg_frame->data, jpeg_frame->size, NULL);
            if (ret < 0) {
                error_cnt++;
                LOG_E("avi write error, sd cnt:%d, err_cnt:%d, err_code:%d, offset %d\r\n", save_cnt, error_cnt, ret, avi_hd->new_chunk_offset);
            }
...
{
    if (soln_fatfs_sd_init() < 0) {
        LOG_E("sd_card init failed\r\n");
        vTaskDelete(NULL);
    }

    if (g_fatfs_fs.csize * 512 < (16 * 512)) {
        LOG_E("fatfs cluster size %d\r\n", g_fatfs_fs.csize * 512);
        LOG_E("The cluster size must exceed %d\r\n", g_fatfs_fs.csize * 512);
        vTaskDelete(NULL);
    }

    if (g_fatfs_fs.fs_type != FS_EXFAT) {
        LOG_E("the Filesystem type must be exFat (>4G)\r\n");
        vTaskDelete(NULL);
    }

    LOG_I("soln_save_avi_to_sdcard_init\r\n");
    LOG_I("rm -rf /sd/avi_test ...\r\n");

    /* delete dir */
    soln_filedir_delete("/sd/avi_test");
    LOG_I("rm OK!\r\n");
    /* create dir */
    f_mkdir("/sd/avi_test");

#if IS_ENABLED(CONFIG_SOLN_SD_AVI_AUDIO_EN)
    /* Create the AVI output subscription on the remote PCM queue. */
    remote_pcm_output_avi_id = CONFIG_SOLN_FBQ_AUD_PCM_REMOTE_SD_ID;
    if (fbq_output_open(soln_fbq_aud_pcm_remote(), &remote_pcm_output_avi_id,
                        CONFIG_SOLN_FBQ_AUD_PCM_REMOTE_SD_DEPTH,
                        SOLUTION_FBQ_TYPE_MASK_AUDIO_PCM_LOOPBACK) != FBQ_OK) {
        LOG_E("remote PCM AVI output subscription create failed\r\n");
        vTaskDelete(NULL);
    } else {
        LOG_I("remote PCM AVI output ID: %d\r\n", remote_pcm_output_avi_id);
    }

#endif

#if IS_ENABLED(CONFIG_SOLN_SD_AVI_VIDEO_EN)
    uint32_t mjpeg_accept_mask;

    /* select jpeg queue source */
#if IS_ENABLED(CONFIG_SOLN_FBQ_VID_JPEG_REMOTE_EN)
    mjpeg_queue_ctrl_avi = soln_fbq_vid_jpeg_remote();
    jpeg_output_avi_id = CONFIG_SOLN_FBQ_VID_JPEG_REMOTE_SD_ID;
    mjpeg_queue_depth_avi = CONFIG_SOLN_FBQ_VID_JPEG_REMOTE_SD_DEPTH;
    mjpeg_accept_mask = SOLUTION_FBQ_TYPE_MASK_IMG_JPEG_HB_REC;
#elif IS_ENABLED(CONFIG_SOLN_FBQ_VID_JPEG_LOCAL_EN)
    mjpeg_queue_ctrl_avi = soln_fbq_vid_jpeg_local();
    jpeg_output_avi_id = CONFIG_SOLN_FBQ_VID_JPEG_LOCAL_SD_ID;
    mjpeg_queue_depth_avi = CONFIG_SOLN_FBQ_VID_JPEG_LOCAL_SD_DEPTH;
    mjpeg_accept_mask = SOLUTION_FBQ_TYPE_MASK_IMG_JPEG_LOCAL_ALL;
#else
#error "please config mjpeg queue for sd avi"
#endif

    /* create jpeg output queue */
    if (fbq_output_open(mjpeg_queue_ctrl_avi, &jpeg_output_avi_id,
                        mjpeg_queue_depth_avi,
                        mjpeg_accept_mask) != FBQ_OK) {
        LOG_E("jpeg frame sd_avi out queue create failed\r\n");
        vTaskDelete(NULL);
    } else {
        LOG_I("JPEG AVI output ID: %d\r\n", jpeg_output_avi_id);
    }
#endif
...
#elif IS_ENABLED(CONFIG_SOLN_SD_AVI_VIDEO_EN)
    /* onyl vids */
...
int soln_save_avi_to_sdcard_init(void)
{
    LOG_I("sdcard_avi_init_task_init\r\n");
