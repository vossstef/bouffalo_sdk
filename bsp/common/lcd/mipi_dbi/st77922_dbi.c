...
#include "../lcd.h"
#include "st77922_dbi.h"

#if defined(LCD_DBI_ST77922)
...
#if (ST77922_DBI_PIXEL_FORMAT == 1)
    .pixel_format = LCD_DBI_LCD_PIXEL_FORMAT_RGB565,
#elif (ST77922_DBI_PIXEL_FORMAT == 2)
...
const st77922_dbi_init_cmd_t st77922_dbi_init_cmds[] = {
    { 0xF1, "\x00", 1 },
    { 0x60, "\x00\x00\x00", 3 },
    { 0x65, "\x80", 1 },
    { 0x79, "\x06", 1 },
    ...
    { 0xF0, "\x00", 1 },
    { 0xD0, "\x00", 1 },
    { 0x2A, "\x00\x00\x01\x3F", 4 }, /* column: 0 ~ 319 */
    { 0x2B, "\x00\x00\x01\xDF", 4 }, /* row: 0 ~ 479 */
...
/* Color reversal */
#if defined(ST77922_DBI_COLOR_REVERSAL) && ST77922_DBI_COLOR_REVERSAL
    { 0x21, NULL, 0 },
#else
    { 0x20, NULL, 0 },
#endif
...
/**
 * @brief st77922_dbi_async_callback_enable
 *
 * @return
 */
void st77922_dbi_async_callback_enable(bool enable)
{
    lcd_dbi_sync_callback_enable(enable);
}

/**
 * @brief st77922_dbi_async_callback_register
 *
 * @return
 */
void st77922_dbi_async_callback_register(void (*callback)(void))
{
    lcd_dbi_async_callback_register(callback);
}

/**
 * @brief st77922_dbi_draw_is_busy, After the call st77922_dbi_draw_picture_dma must check this,
 *         if st77922_dbi_draw_is_busy() == 1, Don't allow other draw !!
 *         can run in the DMA interrupt callback function.
 *
 * @return int 0:draw end; 1:Being draw
 */
int st77922_dbi_draw_is_busy(void)
{
    return lcd_dbi_isbusy();
}

/**
 * @brief st77922_dbi_init
 *
 * @return int
 */
int st77922_dbi_init()
{
    lcd_dbi_init(&dbi_para);

    for (uint16_t i = 0; i < (sizeof(st77922_dbi_init_cmds) / sizeof(st77922_dbi_init_cmd_t)); i++) {
        if (st77922_dbi_init_cmds[i].cmd == 0xFF && st77922_dbi_init_cmds[i].data == NULL && st77922_dbi_init_cmds[i].databytes) {
            bflb_mtimer_delay_ms(st77922_dbi_init_cmds[i].databytes);
        } else {
            lcd_dbi_transmit_cmd_para(st77922_dbi_init_cmds[i].cmd, (void *)(st77922_dbi_init_cmds[i].data), st77922_dbi_init_cmds[i].databytes);
        }
    }

    st77922_dbi_set_dir(0, 0);
    return 0;
}
 ...
 * @param mir_flag
 */
int st77922_dbi_set_dir(uint8_t dir, uint8_t mir_flag)
{
...
#if defined(ST77922_DBI_DIR_MIRROR) && ST77922_DBI_DIR_MIRROR
    mir_flag = !mir_flag;
#endif
...
/* Color RGB order */
#if defined(ST77922_DBI_COLOR_ORDER) && ST77922_DBI_COLOR_ORDER
    param |= 0x08;
#endif
...
/**
 * @brief st77922_dbi_set_draw_window
 *
 * @param x1
 * @param y1
 * @param x2
 * @param y2
 */
void st77922_dbi_set_draw_window(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
{
#if defined(ST77922_DBI_OFFSET_X) && ST77922_DBI_OFFSET_X
    x1 += ST77922_DBI_OFFSET_X;
    x2 += ST77922_DBI_OFFSET_X;
#endif
#if defined(ST77922_DBI_OFFSET_Y) && ST77922_DBI_OFFSET_Y
    y1 += ST77922_DBI_OFFSET_Y;
    y2 += ST77922_DBI_OFFSET_Y;
#endif
...
/**
 * @brief st77922_dbi_draw_point
 *
 * @param x
 * @param y
 * @param color
 */
void st77922_dbi_draw_point(uint16_t x, uint16_t y, st77922_dbi_color_t color)
{
    /* set window */
    st77922_dbi_set_draw_window(x, y, x, y);

    lcd_dbi_transmit_cmd_pixel_sync(0x2C, (void *)&color, 1);
}

/**
 * @brief st77922_dbi_draw_area
 *
 * @param x1
 * @param y1
 * @param x2
 ...
 * @param color
 */
void st77922_dbi_draw_area(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, st77922_dbi_color_t color)
{
    uint32_t pixel_num = (x2 - x1 + 1) * (y2 - y1 + 1);
    uint32_t color_src;

#if ST77922_DBI_COLOR_DEPTH == 16
    color_src = color << 16 | color;
#elif ST77922_DBI_COLOR_DEPTH == 32
    color_src = color;
#endif

    /* set window */
    st77922_dbi_set_draw_window(x1, y1, x2, y2);

    lcd_dbi_transmit_cmd_pixel_fill_sync(0x2C, color_src, pixel_num);
}

/**
 * @brief st77922_dbi_draw_picture_dma, Non-blocking! Using DMA acceleration, Not waiting for the draw end
 *  After the call, No other operations are allowed until (st77922_dbi_draw_is_busy()==0)
 *
 * @param x1
 * @param y1
 * @param x2
...
void st77922_dbi_draw_picture_nonblocking(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, st77922_dbi_color_t *picture)
{
    uint32_t pixel_num = (x2 - x1 + 1) * (y2 - y1 + 1);
...
/**
 * @brief st77922_dbi_draw_picture,Blocking，Using DMA acceleration,Waiting for the draw end
 *
 * @param x1
 * @param y1
 * @param x2
 ...
 * @param picture
 */
void st77922_dbi_draw_picture_blocking(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, st77922_dbi_color_t *picture)
{
    uint32_t pixel_num = (x2 - x1 + 1) * (y2 - y1 + 1);
