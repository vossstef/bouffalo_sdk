...
- [本文件](AGENTS.md) - 项目整体概览和环境设置
- [WiFi 驱动组件](components/wireless/wl80211/CLAUDE.md) - wl80211 WiFi 驱动详细文档

## Agent Skills

Read the applicable skill before starting work in these areas:

- [Bouffalo SDK development](.agents/skills/bouffalo-sdk/SKILL.md): SDK architecture, build configuration, peripheral, wireless, power, and debugging work.
- [SDK changelog](.agents/skills/sdk-changelog/SKILL.md): customer-facing release notes.
- [Manual testing](.agents/skills/test-manual/SKILL.md): querying, building, and running manual Jenkins test cases.

### SDK Release Changelog

For an SDK release, update the root `CHANGELOG.md` using the `sdk-changelog` skill.

- Collect commits from `<previous-release-tag>..HEAD` in the main repository.
- Compare gitlinks at the release tag and `HEAD`, then collect the corresponding commits from every changed, checked-out submodule.
- Also inspect `components/wireless/wl80211/src`, which is an embedded Git repository rather than a gitlink; derive its baseline commit from the previous tag date.
- Exclude bookkeeping-only submodule-record commits, deduplicate equivalent changes, and group entries by functional area.
- Write customer-facing entries only: omit commit IDs, Jira IDs, authors, internal paths, and implementation-only identifiers. Omit documentation-only changes unless they correct user-visible behavior.
- Use the established release heading format: `## vX.Y.Z — since vA.B.C (YYYY-MM-DD → YYYY-MM-DD)`.

### Manual Jenkins Tests

Use `./tools/CI/test_manual show` to identify an exact case or suite. Running a hardware test triggers an external Jenkins job, so do not run it unless the user explicitly authorizes the case, SDK path, and target agent. Use `--build-only` when only a build verification is needed.
...
- [本文件](AGENTS.md) - 环境设置和项目概览
- [WiFi 驱动文档](components/wireless/wl80211/CLAUDE.md) - wl80211 详细 API
- [Bouffalo SDK 开发指南](.agents/skills/bouffalo-sdk/SKILL.md) - AI 辅助开发技能
- [SDK 发布说明规范](.agents/skills/sdk-changelog/SKILL.md) - 面向用户的 CHANGELOG 生成规则
- [手工测试规范](.agents/skills/test-manual/SKILL.md) - Jenkins 手工测试流程
