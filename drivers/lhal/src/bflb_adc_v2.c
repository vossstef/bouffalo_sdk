...
enum bflb_adc_init_state {
    ADC_INIT_STATE_UNINIT = 0,
    ADC_INIT_STATE_INITIALIZING,
    ADC_INIT_STATE_READY,
};

static volatile uint8_t adc_init_state;
static volatile uint8_t adc_inject_context_owned;
static uint32_t adc_inject_context_token;

#define ADC_CONTEXT_CMD1_MASK (GLB_GPADC_TS_EN | GLB_GPADC_TSVBE_LOW | GLB_GPADC_TSEXT_SEL | GLB_GPADC_INT_NEG_SEL_MASK | GLB_GPADC_INT_POS_SEL_MASK)

#define ADC_CONTEXT_CONFIG1_MASK \
    (GLB_GPADC1_VREF_EX_SEL | GLB_GPADC1_DIFF_MODE | GLB_GPADC1_NEG_GND | GLB_GPADC1_CHOP_MODE_MASK | GLB_GPADC1_INT_INSERT_LEN_MASK | GLB_GPADC1_EXT_INSERT_LEN_MASK | GLB_GPADC1_INT_EXT_INSERT_SEL)

#define ADC_CONTEXT_CONFIG2_MASK (GLB_GPADC1_CONT_CONV_EN | GLB_GPADC1_SCN_EN | GLB_GPADC1_SCN_LOOP_EN | GLB_GPADC1_RES_SEL_MASK)

#define ADC_CONTEXT_STATUS_MASK  (1U << GLB_GPADC1_RESERVED_SHIFT)

#define ADC_CONTEXT_GPIP2_MASK   (GPIP_GPADC_DMA_EN | GPIP_GPADC_RDY_MASK | GPIP_GPADC_FIFO_OVERRUN_MASK | GPIP_GPADC_FIFO_UNDERRUN_MASK | GPIP_GPADC_FIFO_THL_MASK | GPIP_GPADC_INT_RDY_NUM_MASK)

static void bflb_adc_apply_config(struct bflb_device_s *dev, const struct bflb_adc_config_s *config)
{
    uint32_t regval;
    uint32_t reg_base = dev->reg_base;

    regval = getreg32(reg_base + GLB_GPADC1_REG_CONFIG1_OFFSET);
    regval &= ~(GLB_GPADC1_VREF_EX_SEL | GLB_GPADC1_DIFF_MODE | GLB_GPADC1_NEG_GND | GLB_GPADC1_CHOP_MODE_MASK);
    ...
    if (config->resolution == ADC_RESOLUTION_12B) {
        regval |= (1 << GLB_GPADC1_CHOP_MODE_SHIFT);
    } else {
        regval |= (2 << GLB_GPADC1_CHOP_MODE_SHIFT);
    }
    putreg32(regval, reg_base + GLB_GPADC1_REG_CONFIG1_OFFSET);

    regval = getreg32(reg_base + GLB_GPADC1_REG_CONFIG2_OFFSET);
    regval &= ~(GLB_GPADC1_RES_SEL_MASK | GLB_GPADC1_SCN_LOOP_EN | GLB_GPADC1_SCN_EN | GLB_GPADC1_CONT_CONV_EN);
    regval |= (config->resolution << GLB_GPADC1_RES_SEL_SHIFT);
    if (config->scan_conv_mode) {
        regval |= GLB_GPADC1_SCN_EN;
        if (config->continuous_conv_mode) {
            regval |= GLB_GPADC1_SCN_LOOP_EN;
        }
    } else if (config->continuous_conv_mode) {
        regval |= GLB_GPADC1_CONT_CONV_EN;
    }
    putreg32(regval, reg_base + GLB_GPADC1_REG_CONFIG2_OFFSET);
}

static void bflb_adc_restore_reg(uint32_t addr, uint32_t saved_value, uint32_t mask)
{
    uint32_t regval;

    regval = getreg32(addr);
    regval &= ~mask;
    regval |= saved_value & mask;
    putreg32(regval, addr);
}

static void bflb_adc_init_complete(void)
{
    uintptr_t irq_state;

    irq_state = bflb_irq_save();
    adc_init_state = ADC_INIT_STATE_READY;
    bflb_irq_restore(irq_state);
}

static void bflb_adc_reset_state(void)
{
    uintptr_t irq_state;

    irq_state = bflb_irq_save();
    adc_init_state = ADC_INIT_STATE_UNINIT;
    adc_inject_context_owned = 0;
    bflb_irq_restore(irq_state);
}
...
int bflb_adc_init(struct bflb_device_s *dev, const struct bflb_adc_config_s *config)
{
    ...
    irq_state = bflb_irq_save();
    if (adc_init_state == ADC_INIT_STATE_INITIALIZING) {
        bflb_irq_restore(irq_state);
        return -EBUSY;
    }

    reconfigure_only = (adc_init_state == ADC_INIT_STATE_READY) && ((getreg32(dev->reg_base + GLB_GPADC1_REG_CONFIG2_OFFSET) & GLB_GPADC1_GLOBAL_EN) != 0);
    adc_init_state = ADC_INIT_STATE_INITIALIZING;
    bflb_irq_restore(irq_state);

    if (reconfigure_only) {
        bflb_adc_apply_config(dev, config);
        bflb_adc_init_complete();
        return 0;
    }
    ...
    regval |= (GPIP_GPADC_FIFO_UNDERRUN_MASK | GPIP_GPADC_FIFO_OVERRUN_MASK | GPIP_GPADC_RDY_MASK | GPIP_GPADC_FIFO_UNDERRUN_CLR | GPIP_GPADC_FIFO_OVERRUN_CLR | GPIP_GPADC_RDY_CLR);
    ...
    regval = getreg32(ADC_GPIP_BASE + GPIP_GPADC_CONFIG_OFFSET);
    regval &= ~(GPIP_GPADC_FIFO_UNDERRUN_CLR | GPIP_GPADC_FIFO_OVERRUN_CLR | GPIP_GPADC_RDY_CLR | GPIP_GPADC_FIFO_CLR);
    putreg32(regval, ADC_GPIP_BASE + GPIP_GPADC_CONFIG_OFFSET);
...
int bflb_adc_channel_config_internal(struct bflb_device_s *dev, struct bflb_adc_channel_s *chan, uint8_t channels)
{
    ...
    if (channels != 1 || getreg32(reg_base + GLB_GPADC1_REG_CONFIG2_OFFSET) & GLB_GPADC1_SCN_EN) {
        return -EINVAL;
    }
...
int bflb_adc_inject_context_save(struct bflb_device_s *dev, struct bflb_adc_inject_context_s *context)
{
    ...
    reg_base = dev->reg_base;
    irq_state = bflb_irq_save();
    gpip2_config = getreg32(ADC_GPIP2_BASE + GPIP_GPADC_CONFIG_OFFSET);
    restore_config = (adc_init_state == ADC_INIT_STATE_READY) && ((getreg32(reg_base + GLB_GPADC1_REG_CONFIG2_OFFSET) & GLB_GPADC1_GLOBAL_EN) != 0);
    if (adc_inject_context_owned || (adc_init_state == ADC_INIT_STATE_INITIALIZING) || (restore_config && ((gpip2_config & GPIP_GPADC_FIFO_DATA_COUNT_MASK) != 0))) {
        bflb_irq_restore(irq_state);
        return -EBUSY;
    }

    adc_inject_context_owned = 1;
    adc_inject_context_token++;
    if (adc_inject_context_token == 0) {
        adc_inject_context_token++;
    }
    context->token = adc_inject_context_token;
    context->restore_config = restore_config;
    bflb_irq_restore(irq_state);

    context->adc_cmd1 = getreg32(reg_base + GLB_GPADC_REG_CMD1_OFFSET);
    context->adc_config1 = getreg32(reg_base + GLB_GPADC1_REG_CONFIG1_OFFSET);
    context->adc_config2 = getreg32(reg_base + GLB_GPADC1_REG_CONFIG2_OFFSET);
    context->adc_status = getreg32(reg_base + GLB_GPADC1_REG_STATUS_OFFSET);
    context->inject_pos1 = getreg32(reg_base + GLB_GPADC_REG_INSERT_POS1_OFFSET);
    context->inject_pos2 = getreg32(reg_base + GLB_GPADC_REG_INSERT_POS2_OFFSET);
    context->inject_neg1 = getreg32(reg_base + GLB_GPADC_REG_INSERT_NEG1_OFFSET);
    context->inject_neg2 = getreg32(reg_base + GLB_GPADC_REG_INSERT_NEG2_OFFSET);
    context->internal_inject_pos = getreg32(reg_base + GLB_GPADC_INT_INSERT_POS_OFFSET);
    context->internal_inject_neg = getreg32(reg_base + GLB_GPADC_INT_INSERT_NEG_OFFSET);
    context->gpip2_config = gpip2_config;
...
int bflb_adc_inject_context_restore(struct bflb_device_s *dev, const struct bflb_adc_inject_context_s *context)
{
    ...
    if ((dev == NULL) || (context == NULL) || (context->token == 0)) {
        return -EINVAL;
    }

    irq_state = bflb_irq_save();
    if (!adc_inject_context_owned || (context->token != adc_inject_context_token)) {
        bflb_irq_restore(irq_state);
        return -EINVAL;
    }
    bflb_irq_restore(irq_state);

    bflb_adc_clear_fifo_inject(dev);
    if (context->restore_config) {
        reg_base = dev->reg_base;
        putreg32(context->inject_pos1, reg_base + GLB_GPADC_REG_INSERT_POS1_OFFSET);
        putreg32(context->inject_pos2, reg_base + GLB_GPADC_REG_INSERT_POS2_OFFSET);
        putreg32(context->inject_neg1, reg_base + GLB_GPADC_REG_INSERT_NEG1_OFFSET);
        putreg32(context->inject_neg2, reg_base + GLB_GPADC_REG_INSERT_NEG2_OFFSET);
        putreg32(context->internal_inject_pos, reg_base + GLB_GPADC_INT_INSERT_POS_OFFSET);
        putreg32(context->internal_inject_neg, reg_base + GLB_GPADC_INT_INSERT_NEG_OFFSET);
        bflb_adc_restore_reg(reg_base + GLB_GPADC_REG_CMD1_OFFSET, context->adc_cmd1, ADC_CONTEXT_CMD1_MASK);
        bflb_adc_restore_reg(reg_base + GLB_GPADC1_REG_STATUS_OFFSET, context->adc_status, ADC_CONTEXT_STATUS_MASK);
        bflb_adc_restore_reg(reg_base + GLB_GPADC1_REG_CONFIG1_OFFSET, context->adc_config1, ADC_CONTEXT_CONFIG1_MASK);
        bflb_adc_restore_reg(reg_base + GLB_GPADC1_REG_CONFIG2_OFFSET, context->adc_config2, ADC_CONTEXT_CONFIG2_MASK);
        bflb_adc_restore_reg(ADC_GPIP2_BASE + GPIP_GPADC_CONFIG_OFFSET, context->gpip2_config, ADC_CONTEXT_GPIP2_MASK);
    }
...
float bflb_adc_tsen_raw_to_temperature(struct bflb_device_s *dev, uint32_t rawdata_P, uint32_t rawdata_N)
{
    ...
    (void)dev;
    delta = ((float)(rawdata_N & 0xFFFF) - (float)(rawdata_P & 0xFFFF)) / coe;
    delta = (delta >= 0.0f) ? delta : -delta;
    return (delta - (float)tsen_offset) / 8.488f;
}
...
int bflb_adc_feature_control(struct bflb_device_s *dev, int cmd, size_t arg)
{
    ...
    switch (cmd) {
        ...
        case ADC_CMD_TSEN_EN:
            regval = getreg32(reg_base + GLB_GPADC_REG_CMD1_OFFSET);
            if (arg) {
                regval |= (GLB_GPADC_TS_EN | GLB_GPADC_TSVBE_LOW);
            } else {
                regval &= ~(GLB_GPADC_TS_EN | GLB_GPADC_TSVBE_LOW);
            }
            putreg32(regval, reg_base + GLB_GPADC_REG_CMD1_OFFSET);
